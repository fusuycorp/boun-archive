    ...
    <table class="w-full text-left border-collapse">
      <thead>
        <tr class="border-b border-[#dbd7cc] bg-[#e7e4d9]/60 dark:border-[#27272a] dark:bg-[#121214]">
          ...
          <th class="p-4 text-xs font-semibold text-[#746f65] dark:text-neutral-500 uppercase tracking-wider">Schedule & Rooms</th>
      ...
      <tbody class="divide-y divide-[#dbd7cc]/70 dark:divide-[#27272a]">
        {#each results as course}
          <tr class="hover:bg-[#edeae0] transition-colors group dark:hover:bg-[#232328]">
              ...
              <div class="flex items-center space-x-1.5">
                ...
                {#if course.instructor && course.instructor !== 'TBA'}
                  <a 
                    ...
                    class="hover:text-[#002d72] dark:hover:text-amber-400 hover:underline truncate max-w-[140px]"
                  >
                    {course.instructor}
                  </a>
                {:else}
            ...
            <td class="p-4">
              <div class="flex flex-wrap gap-1.5 max-w-xs">
                {#if course.slots && course.slots.length > 0}
                  ...
                  {/each}
                {:else}
                  <span class="text-[#8a857a] text-xs italic">No schedule</span>
                {/if}
              </div>
            </td>
