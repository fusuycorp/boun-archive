...
<div class="space-y-8">
  <div>
    <h2 class="text-3xl font-bold text-slate-800">Trend Engine</h2>
    <p class="text-slate-500 mt-2">Predictive course scheduling based on 50 years of data.</p>
  </div>

  <div class="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm space-y-6">
    <div class="max-w-md">
      <label class="text-xs font-bold text-slate-400 uppercase tracking-widest">Enter Course Code</label>
      <div class="flex mt-2 space-x-2">
        <input 
          type="text" 
          bind:value={query}
          placeholder="e.g. INTT514"
          class="flex-1 p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 font-mono font-bold"
        />
        <button 
    ...
    {#if error}
      <div class="p-4 bg-red-50 text-red-600 rounded-xl flex items-center space-x-3 border border-red-100">
        <AlertCircle size={20} />
            ...
            {#each Object.entries(prediction.offering_probability) as [sem, prob]}
              <div>
                <div class="flex justify-between text-sm font-bold mb-1">
                  <span class="text-slate-600">{sem}</span>
                  <span class="text-indigo-600">{Number(prob).toFixed(1)}%</span>
                </div>
            ...
            {#each prediction.predicted_slots as slot}
              <div class="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-100">
                <div class="flex items-center space-x-4">
                  <div class="w-10 h-10 bg-white rounded-lg flex items-center justify-center font-bold text-indigo-700 border border-slate-200">
                    {slot.day}
                  </div>
                  <div>
                    <div class="text-sm font-bold text-slate-800">Hour {slot.hour}</div>
                    <div class="text-xs text-slate-500">Historical Confidence</div>
                  </div>
                </div>
                <div class="text-right">
                  <div class="text-lg font-black text-indigo-600">{(slot.confidence_score * 100).toFixed(0)}%</div>
                </div>
              </div>
            {/each}
          </div>
