...
<div class="max-w-6xl mx-auto space-y-6 sm:space-y-8">
  <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
    <div class="flex items-center space-x-3 sm:space-x-4">
      <a href="/instructors" class="p-2 bg-white border border-slate-200 rounded-xl text-slate-400 hover:text-indigo-600 hover:border-indigo-100 transition-all dark:bg-slate-900 dark:border-slate-800 shadow-2xs" aria-label="Back to instructors">
        <ArrowLeft size={18} />
      </a>
      <div>
    ...
    {#if legacyData}
       <button 
          ...
          class="flex items-center justify-center space-x-2 bg-white border border-slate-200 text-slate-600 px-4 py-2 rounded-xl text-xs font-bold hover:bg-slate-50 transition-colors shadow-xs dark:bg-slate-800 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-700 cursor-pointer w-full sm:w-auto"
       >
         <Download size={14} />
  ...
  {:else if error}
    <div class="bg-white rounded-3xl border-2 border-dashed border-red-200 p-12 sm:p-24 flex flex-col items-center justify-center text-center dark:bg-slate-900 dark:border-red-900/30">
      ...
      <a href="/instructors" class="mt-6 px-6 sm:px-8 py-2.5 sm:py-3 bg-indigo-600 text-white rounded-2xl text-xs sm:text-sm font-bold shadow-md shadow-indigo-200 dark:shadow-none hover:bg-indigo-700 transition-colors">Back to Search</a>
    </div>
  {:else if legacyData}
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
      <!-- Stats Summary -->
      <div class="lg:col-span-1 space-y-6">
        <div class="bg-white p-5 sm:p-8 rounded-2xl sm:rounded-3xl border border-slate-200 shadow-xs space-y-6 dark:bg-slate-900 dark:border-slate-800">
          <div class="flex flex-col items-center text-center">
             <div class="w-16 sm:w-20 h-16 sm:h-20 bg-indigo-50 dark:bg-indigo-950/40 rounded-full flex items-center justify-center text-indigo-600 dark:text-indigo-400 mb-3 sm:mb-4">
          ...
          <div class="grid grid-cols-2 gap-4 border-t border-slate-50 dark:border-slate-800/60 pt-6">
            <div class="text-center">
              <div class="text-xl sm:text-2xl font-black text-indigo-600 dark:text-indigo-400">{legacyData.total_semesters_taught}</div>
              <div class="text-[9px] sm:text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase">Semesters</div>
            </div>
            <div class="text-center">
              <div class="text-xl sm:text-2xl font-black text-indigo-600 dark:text-indigo-400">{legacyData.total_courses_taught}</div>
        ...
        <!-- Preferred Slots -->
        <div class="bg-white p-5 sm:p-6 rounded-2xl border border-slate-200 shadow-xs space-y-4 dark:bg-slate-900 dark:border-slate-800">
      ...
      <div class="lg:col-span-2 space-y-6">
        <div class="bg-white p-5 sm:p-8 rounded-2xl sm:rounded-3xl border border-slate-200 shadow-xs dark:bg-slate-900 dark:border-slate-800">
          <h4 class="text-xs sm:text-sm font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest flex items-center space-x-2 mb-4 sm:mb-6">
            <History size={16} class="text-indigo-500 dark:text-indigo-400" />
          ...
          <div class="space-y-3 max-h-[500px] sm:max-h-[600px] overflow-y-auto pr-1 custom-scrollbar">
            {#each legacyData.history as item}
              <div class="flex items-start space-x-3 sm:space-x-4 p-3 sm:p-4 rounded-xl border border-slate-50 hover:border-indigo-100 hover:bg-indigo-50/30 transition-all dark:border-slate-800/40 dark:hover:border-indigo-950/40 dark:hover:bg-indigo-950/15">
                <div class="text-[11px] sm:text-xs font-black text-indigo-600 bg-white px-2.5 sm:px-3 py-1 rounded-full border border-indigo-100 whitespace-nowrap dark:bg-slate-900 dark:border-indigo-900/40 dark:text-indigo-400 font-mono">
                  {item.term}
                </div>
                <div class="flex-1 min-w-0">
                  <a href="/course/{item.course_code}" class="text-xs sm:text-sm font-bold text-slate-800 dark:text-slate-200 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors block truncate">
                    {item.course_code}
                  </a>
        ...
        <div class="bg-white p-5 sm:p-8 rounded-2xl sm:rounded-3xl border border-slate-200 shadow-xs dark:bg-slate-900 dark:border-slate-800">
           <h4 class="text-xs sm:text-sm font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest flex items-center space-x-2 mb-4 sm:mb-6">
            <BookOpen size={16} class="text-indigo-500 dark:text-indigo-400" />
            <span>Most Frequent Courses</span>
          </h4>
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            {#each Object.entries(legacyData.most_frequent_courses) as [code, count]}
               <div class="flex items-center justify-between p-3 sm:p-4 bg-slate-50 rounded-xl border border-slate-100 dark:bg-slate-950 dark:border-slate-850">
                  <a href="/course/{code}" class="text-xs sm:text-sm font-black text-slate-700 dark:text-slate-350 hover:text-indigo-600 transition-colors">{code}</a>
                  <div class="flex items-center space-x-1.5">
                    <div class="text-[9px] text-slate-400 dark:text-slate-500 font-bold uppercase">Taught</div>
                    <span class="text-base sm:text-lg font-black text-indigo-600 dark:text-indigo-400">{count}x</span>
                  </div>
               </div>
            {/each}
          </div>
