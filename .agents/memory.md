...
- The GitHub repo moved from `fusuyfusuy/boun-archive` to `fusuycorp/boun-archive` (org rename/transfer, discovered 2026-08-17 via a "repository moved" notice on push). Local `origin` remote was updated to `git@github.com:fusuycorp/boun-archive.git` — if a fresh clone or another machine still points at `fusuyfusuy/...`, update it the same way rather than relying on GitHub's redirect.
- `frontend/src/app.html` loads Umami analytics site-wide via a `<script defer src="https://umami.bogazici.app/script.js" data-website-id="f58e2223-ae5b-4963-9663-2bb7d0da6cb3">` tag in `<head>`, placed before `%sveltekit.head%`. Keep this when touching app.html.
- `scripts/sync_from_scraper.py` is the designated live ingestion script for `boun-scrape` feeds. Never use `scripts/migrate_to_pg.py` for live sync (it executes `Base.metadata.drop_all(engine)`). Course matching in `sync_from_scraper.py` is done via composite natural key `(term_id, dept_kisaadi, course_code, section)`.
- Campus topography: `JF` (John Freely Hall) is located on **Güney Kampüs** (South Campus), whereas `EF` (Faculty of Education / Eğitim Fakültesi) is on **Kuzey Kampüs** (North Campus). Do not swap them when mapping rooms or calculating inter-campus commute constraints.
- `bountools.com` is the live public domain for the platform (`https://bountools.com`). Keep `bountools.com` as canonical in public environment variable templates and routing examples.
- Public documentation hygiene: Never leak private infrastructure node names (`tanri`, `worky`), internal orchestration IDs (`FnoW3VW_TLpX8sXKPL60v`), or local user paths (`/home/devhax/...`, `fusuyfusuy`) in public-facing markdown documents.
